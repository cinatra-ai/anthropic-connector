...
// DUPLICATED at core `@cinatra-ai/llm src/log-redaction.ts` and at
// `@cinatra-ai/openai-connector src/log-redaction.ts` (same content). Sharing it
// is blocked by the dep direction (the ABI leaf is TYPE-only over the serverEntry
// graph, and reverse-importing the host `packages/llm` would cycle); ~15 LoC is
// cheap enough to duplicate, and each copy is exercised by its own vitest canary.
