...
export async function getConfiguredAnthropicConnection(): Promise<{
  ...
  if (deps.nango.isConfigured()) {
    ...
    if (credentials && typeof credentials === "object" && "apiKey" in credentials && typeof credentials.apiKey === "string") {
      return {
...
export function getAnthropicAPIStatus() {
  ...
  const savedConnection = getAnthropicDeps().nango.getPrimarySavedConnection("claude");
...
export async function saveAnthropicAPISettings(input: {
  ...
  const deps = getAnthropicDeps();
  const current = getAnthropicAPISettings();
  ...
  await deps.nango.ensureIntegration({
    provider: "anthropic",
    providerConfigKey: deps.nango.providerConfigKeys.claude,
  ...
  await deps.nango.importConnection({
    connectorKey: "claude",
    providerConfigKey: deps.nango.providerConfigKeys.claude,
    connectionId: deps.nango.connectionIds.claude,
    credentials: {
      type: "API_KEY",
  ...
  // Clear local apiKey after syncing to Nango. Merge the FULL stored settings
  // (defaultModel / mcpMode / promptCachingEnabled) instead of writing a bare
