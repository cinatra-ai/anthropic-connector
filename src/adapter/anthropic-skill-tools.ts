 ...
 *
 * Relocated verbatim into the connector (llm-providers S4 — cinatra#1715): the
 * only deltas are the two import rewires — the neutral tool TYPES now come from
 * the sdk-extensions ABI leaf (type-only over the serverEntry graph), and the
 * error VALUES from the connector-inlined `./adapter-errors`. These are
 * request-TRANSLATION primitives (they consume already-built neutral tools and
 * emit Anthropic request params); the separate skill-delivery/-sync seam that
 * BUILDS the container_skills tool stays in core (cinatra#1964).
 */
