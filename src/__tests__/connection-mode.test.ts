...
import { registerAnthropicConnector, _resetAnthropicDepsForTests, type AnthropicConnectorDeps } from "../deps";
import {
...
describe("persistence + resolved transport (through the deps store)", () => {
  ...
  it("ignores a corrupt persisted value (fail-closed to unset → API)", () => {
    const store: Record<string, unknown> = { "anthropic-connection-mode": { mode: "shell" } };
...
describe("decideConnectionModeWrite (server-side write policy — AC2)", () => {
  ...
  it("persists a deliberate localCli → api switch (the hydrated select has no no-loss trap — AC4 transition)", () => {
    // The select is hydrated to the resolved transport, so an `api` submit is the
    // admin's actual choice to leave local-CLI — it MUST persist, never be dropped.
