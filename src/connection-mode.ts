// The Anthropic connector's API-vs-local-CLI connection mode (cinatra#1926,
// epic #1873 M5).
//
...
const CONNECTION_MODE_STORE_KEY = "anthropic-connection-mode";
...
export function getPersistedConnectionMode(): ConnectionMode | undefined {
  const stored = getAnthropicDeps().readConnectorConfigFromDatabase<StoredConnectionMode>(
...
export function saveConnectionMode(mode: ConnectionMode): void {
  getAnthropicDeps().writeConnectorConfigToDatabase(CONNECTION_MODE_STORE_KEY, {
    mode,
 ...
 * reaches to dispatch `@claude`. Reads the persisted mode and applies the
...
export function getResolvedConnectionTransport(): ConnectionMode {
  return resolveConnectionTransport(
    ...
    getAnthropicDeps().localCliEligible(),
  );
}
